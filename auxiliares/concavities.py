...
def processa_imagem(imagem, limiar=127, tamanho_subimagem=50, sobreposicao=0.5, 
                   ...
                   caminho_saida_terceira="saida_xor.jpg"):
    """
    Processa a imagem para identificar concavidades e traçar linhas desde os pontos de concavidade até a linha do meio,
    estendendo-as a partir dos pontos de concavidade. Retorna três imagens: uma com as linhas na imagem original,
    outra com as linhas na imagem dilatada, e uma terceira com o bitwise XOR dos contornos dessas duas imagens.
    ...
    - caminho_saida_terceira: caminho para salvar a imagem resultante do bitwise XOR.
    
    Retorna:
    - imagem_resultado: imagem original com linhas traçadas.
    - imagem_dilatada_com_linhas: imagem dilatada com linhas estendidas.
    - imagem_xor: imagem resultante do bitwise XOR dos contornos.
    """
    ...
    imagem_dilata_aux = dilata_imagem(imagem, 5, 1)

    # Encontrar contornos na imagem morfologicamente processada
    contornos = encontra_contornos(imagem_dilata_aux)
    if not contornos:
    ...
    # Garantir que imagem_resultado tenha 3 canais (BGR)
    if len(imagem_resultado.shape) == 2:
        imagem_resultado = cv2.cvtColor(imagem_resultado, cv2.COLOR_GRAY2BGR)
    ...
    imagem_dilatada = dilata_imagem(imagem_binaria, tamanho_kernel=9, iteracoes=7)

    imagem_dilatada_com_linhas = cv2.cvtColor(imagem_dilatada, cv2.COLOR_GRAY2BGR)

    for linha in linhas:
        ponto_concavidade, ponto_projetado = linha
        cv2.line(imagem_dilatada_com_linhas, ponto_concavidade, ponto_projetado, (0, 0, 0), 2)
        ...
        cv2.line(imagem_dilatada_com_linhas, ponto_concavidade, novo_ponto, (0, 0, 0), 2)



    imagem_resultado_cinza = cv2.cvtColor(imagem_resultado, cv2.COLOR_BGR2GRAY)
    imagem_dilatada_com_linhas_cinza = cv2.cvtColor(imagem_dilatada_com_linhas, cv2.COLOR_BGR2GRAY)
    imagem_resultado_cinza_bin = binariza_imagem(imagem_resultado_cinza, 127)
    imagem_dilatada_com_linhas_cinza_bin = binariza_imagem(imagem_dilatada_com_linhas_cinza, 127)
    mascara_resultado = cv2.adaptiveThreshold(imagem_resultado_cinza_bin, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 3, 0)
    imagem_xor = cv2.bitwise_xor(mascara_resultado, imagem_dilatada_com_linhas_cinza_bin)

    
    imagem_resultado = remover_componentes(imagem_resultado)
    imagem_xor = remover_componentes(imagem_xor)
    imagem_dilatada_com_linhas = remover_componentes(imagem_dilatada_com_linhas)
    salva_imagem(imagem_resultado, caminho_saida)
    salva_imagem(imagem_xor, caminho_saida_terceira)
    salva_imagem(imagem_dilatada_com_linhas, caminho_saida_segunda)
    '''cv2.imshow("imagem_resultado_cinza",imagem_resultado_cinza)
    cv2.imshow("imagem_dilatada_com_linhas_cinza",imagem_dilatada_com_linhas_cinza)
    cv2.imshow("mascara_resultado",mascara_resultado)
    '''
    return imagem_resultado, imagem_dilatada_com_linhas, imagem_xor

def remover_componentes(imagem, tamanho_minimo = 0, tamanho_maximo = 200):
        if imagem is not None:
            ...
            num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(imagem_binaria, connectivity=4)
            for label in range(1, num_labels):
                area = stats[label, cv2.CC_STAT_AREA]
                if area > tamanho_minimo and area < tamanho_maximo:
